...
foo_rele(struct foo *fp){
    ...
    //引用
    if(fp->f_count == 1){
        pthread_mutex_unlock(&fp->f_lock);//允许其他线程访问该变量
        pthread_mutex_lock(&hashlock); //要回收哈希表了，锁住整个哈希表
        pthread_mutex_lock(&fp->f_lock);
        if(fp->f_count != 1){
            if(fp->f_count>1){
                fp->f_count--;
            }else{
                fprintf(stderr,"unexpected refrence count detected\n");
                if(pthread_mutex_unlock(&fp->f_lock)!=0 || pthread_mutex_unlock(&hashlock)!=0){
